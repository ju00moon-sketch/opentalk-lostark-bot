...
디스코드 슬래시에는 **기본 커맨드 58개**를 등록합니다. 초성·단어 별칭은 기존 채팅 입력(`ㅂㅂㄱ 4000`·`.업뎃`)으로 사용하며 슬래시 목록에는 중복 등록하지 않습니다. **출첵·한마디는 카카오톡 전용**입니다.
...
디스코드에서는 아래 두 커맨드의 슬래시·채팅 입력을 지원하지 않습니다.

| 커맨드 | 기능 |
|---|---|
| `/출첵` | 출석 체크 — 하루 한 번(한국 시간 자정 기준) 1~3P 무작위 지급, 누적 포인트·출석 횟수 표시 (`/출첵`·`.출첵`·`/ㅊㅊ`·`.ㅊㅊ`·접두사 없는 `ㅊㅊ`) |
