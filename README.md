...
| `/쌀` | 쌀값 — 재련 재료(파괴석·수호석·돌파석·융화 재료·파편 주머니·숨결·에스더의 기운) 거래소 최저가 한눈에 + 전일 대비 (5분 캐시, `/쌀값`·`ㅆ`) |
...
| `/유각` `/전각` | 비싼 유물/전설 각인서 TOP 10 + 전일 대비 (등급 고정 단축 커맨드, `/유각 2`로 다음 쪽). `/비싼유각`은 `/유각`과 같은 커맨드 |
| `/생활재료` | 생활 재료 분야별 시세 |
...
**방에서 쓰는 법**: `/` 또는 `.`으로 시작 — `/정보 닉네임` `.ㅂㅂㄱ 4000` `/도움말`. 분배금은 예외로 `ㅂㅂㄱ 4000`·`ㅂㅂㄱ 4000 8`처럼 접두사 없이도 사용할 수 있습니다. 다른 초성은 `/` 또는 `.`이 필요합니다. 카톡 닉네임이 캐릭터명이면 닉네임 생략(디스코드
