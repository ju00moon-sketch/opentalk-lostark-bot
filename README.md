...
| `/스펙업 닉네임` | 스펙업 효율 — 같은 골드 대비 점수 상승량 순으로 디스코드 상위 10개, 카톡 상위 5개. 카톡은 옵션 변경·예상 비용·점수 상승·투자 효율을 나눠 표시하고 전체 보기에서 모든 후보와 변경 후 총점 제공. 로펙 후보·점수에 실시간 시세(5분 캐시) 적용, 0G는 `무료`, 로펙 시세 보충 비용은 `*`. 페온은 `/크리스탈`로 적어 둔 시세로 환산(미설정 시 제외) |
| `/크리스탈 [95개가격]` | 스펙업 페온 환산에 쓰는 크리스탈 시세 조회·설정 — 화폐거래소에서 골드로 블루 크리스탈 95개를 살 때 가격. 설정하면 스펙업의 악세·스톤 비용에 페온 골드 포함 |
