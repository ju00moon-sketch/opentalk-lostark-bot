...
| `/스펙업 닉네임` | 스펙업 효율 — 같은 골드 대비 점수 상승량 순으로 디스코드 상위 10개, 카톡 상위 5개. 카톡은 옵션 변경·예상 비용·점수 상승·투자 효율을 나눠 표시하고 전체 보기에서 모든 후보와 변경 후 총점 제공. 로펙 후보·점수에 실시간 시세(5분 캐시) 적용, 0G는 `무료`, 로펙 시세 보충 비용은 `*`. 페온은 자동 갱신 또는 직접 설정한 크리스탈 시세로 환산(`/크리스탈`로 확인, 시세 미수신 시 제외) |
...
| `/떠상 [서버]` | 떠돌이 상인 — 8개 서버의 전설 호감도 개수·전설 카드 요약과 판매 마감까지 남은 시간. 서버를 넣으면 대륙별 상인·판매 품목과 미제보 대륙 (클로아 커뮤니티 제보 기준, 1분 캐시, `ㄸㅅ`) |
