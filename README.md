...
| `/스펙업 닉네임` | 스펙업 효율 — 같은 골드 대비 점수 상승량 순으로 디스코드 상위 10개, 카톡 상위 5개. 카톡은 옵션 변경·예상 비용·점수 상승·투자 효율을 나눠 표시하고 전체 보기에서 모든 후보와 변경 후 총점 제공. 로펙 후보·점수에 실시간 시세(5분 캐시) 적용, 0G는 `무료`, 로펙 시세 보충 비용은 `*` |
...
서버 닉네임 규칙과 동일), 아니면 `/등록 캐릭터명`(해제 `/등록 해제`). 결과 끝 "이어서 쓸 수 있어요" 아래 커맨드가 디스코드의 버튼 대신이다. 이미지는 폰 봇이
...
`/정보`·`/팔찌`·`/랭킹`·`/체급`은 카톡 전용 줄바꿈과 항목 구분으로 표시한다. 방에 보내는 최종 응답은 후속 안내까지 포함해
