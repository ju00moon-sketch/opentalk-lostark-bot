...
//   GET /p/emo/<키워드>   이모티콘 카드 (가로 미리보기, 페이지 본문에는 원본 이미지)
...
import { emoticonPreviewFor, emoticonCardUrl } from './emoticon-preview.js';
...
function page({ title, description, image, url, displayImage = image, imageSize = null }) {
...
<body><img src="${escapeHtml(displayImage)}" alt="${t}"><h1>${t}</h1><p>${d}</p></body>
</html>
`;
}
...
export async function renderPreview(kind, rawId, { baseUrl, emoticonsEnabled = true }) {
  ...
  if (kind === 'emo') {
    ...
    const original = `${baseUrl}/assets/emoticons/${encodeURIComponent(path.basename(file))}`;
    const preview = emoticonPreviewFor(file);
    const image = preview ? `${baseUrl}/assets/emoticons/preview/${encodeURIComponent(preview.name)}` : original;
    return { status: 200, html: page({ title: id, description: '포근해용 이모티콘', image, displayImage: original,
      imageSize: preview, url: emoticonCardUrl(file, baseUrl, preview) }) };
  }
