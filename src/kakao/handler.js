...
async function runUtterance(utterance, userKey, commandMap, baseUrl, { displayName, guild, roomName = null, limits = CHANNEL_LIMITS.skill } = {}) {
  ...
  const commandText = BARE_QUIP_INPUTS.has(utterance) ? `/${utterance}` : utterance;
  const match = matchTextCommand(commandText, commandMap, KAKAO_MATCH_OPTIONS);
...
// 방에서는 접두사 있는 명령·허용된 단독 명령/초성·[이모티콘에만 반응하고 나머지는 침묵(null).
