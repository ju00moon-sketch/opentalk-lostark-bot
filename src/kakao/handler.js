...
// 카카오 스킬 응답 JSON → 평문 한 덩어리. 이미지는 URL 줄로(폰 봇은 이미지를 못 보냄), 바로가기는 "이어서: …" 한 줄로.
export function flattenResponse(response) {
  const outputs = response?.template?.outputs ?? [];
  const parts = outputs.map((o) => o.simpleText?.text ?? o.simpleImage?.imageUrl ?? '').filter(Boolean);
