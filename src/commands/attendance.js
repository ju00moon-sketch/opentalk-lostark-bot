...
export const data = new SlashCommandBuilder()
  .setName('출첵')
  .setDescription('출석 체크 — 하루 한 번(한국 시간 자정 기준) 1~3P 무작위 지급, 누적 포인트 표시');

export const FAILED = '출석 기록을 저장하지 못했어요. 잠시 후 다시 시도해 주세요';

// 디스코드는 서버 별명(없으면 계정 이름), 카톡은 방 닉네임. 둘 다 없으면 "회원".
...
export function createExecute({ checkIn = checkInStore, now = Date.now } = {}) {
  return async function execute(interaction) {
    ...
    let result;
    try {
      result = checkIn(interaction.user.id, { now }); // 동기 저장이라 deferReply 없이 바로 답한다
    } catch (err) {
    ...
    if (result.status === 'already') {
      await say(`${name}님은 오늘 이미 출석했어요 · 누적 ${result.total}P · 내일 00:00(한국 시간)부터 다시 할 수 있어요`);
      return;
    }
    await say(`✅ ${name}님 출석 완료! +${result.gained}P · 누적 ${result.total}P · ${result.count}번째 출석`);
  };
}
