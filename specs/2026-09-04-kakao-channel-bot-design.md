...
- 범위(사용자 결정): 디스코드 전용 커맨드만 빼고 전부. `/체방` 차트 이미지와 이모티콘(`[따봉`) 포함,
  카카오용 `/등록` 포함.
- 입력 형식(사용자 결정): 처음엔 **`/커맨드` 형식만** 받고 접두사 없는 초성(`ㅈㅂ 닉`)과 `.단어`(`.정보 닉`)는
  스위치로 껐다. **개정 2026-09-05: `.커맨드` 허용. 개정 2026-09-06: `ㅂㅂㄱ`만 접두사 없이 허용. 개정 2026-09-07: `ㅊㅊ`(출첵)도 접두사 없이 허용**
  (`KAKAO_MATCH_OPTIONS.bareAliases`). 그 외 초성 단독과 일반 대화는 여전히 침묵한다. 현행 입력 규칙은
  `specs/2026-09-04-openchat-bridge-design.md`가 기준이다.
...
- 접두사 없는 초성 입력(`ㅂㅂㄱ`·`ㅊㅊ` 제외). `.단어`와 `ㅂㅂㄱ`은 위 입력 형식의 개정으로 허용됐다.
